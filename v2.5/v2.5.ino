/*
  Patched sketch: ESP32 + A7670C: Modbus -> GSM POST (full payload) with two pump inputs
  - Robust modem session handling (reopen on failure)
  - ensureModemSession() before HTTP POST
  - Improved sendJsonPost() with retries & readback
  - Periodic health check + failure counter -> ESP.restart() after repeated failures
  - Keep original web UI & modbus reading behavior
*/
...
// Tank depth in mm (default 7000mm = 7m)
int cfg_tank_depth_mm = 7000;
...
// -------------------- MODEM HEALTH / RECOVERY --------------------
int modemFailureCount = 0;
const int MODEM_FAIL_MAX = 5; // number of consecutive failures before restart
unsigned long lastHealthCheck = 0;
const unsigned long HEALTH_CHECK_INTERVAL = 60000UL; // 60s health check
...
void handleRoot() {
  ...
  String html =
    ...
    "<label>tank_depth_mm</label>"
    "<input type='text' name='tank_depth_mm' value='" + String(cfg_tank_depth_mm) + "' required>"
    ...
    "<p>Payload fields: sensor_id=" + cfg_sensor_id
      + " site_id=" + cfg_site_id
      + " tank_id=" + cfg_tank_id
      + " tank_depth_mm=" + String(cfg_tank_depth_mm) + "</p>"
...
void handleSave() {
  ...
  if (!server.hasArg("sensorUrl") || !server.hasArg("sensor_id") || !server.hasArg("site_id")
      || !server.hasArg("tank_id") || !server.hasArg("tank_depth_mm")
      || !server.hasArg("pump_id") || !server.hasArg("pump_name")
  ...
  // tank depth mm (from form)
  cfg_tank_depth_mm = server.arg("tank_depth_mm").toInt();
  if (cfg_tank_depth_mm <= 0) cfg_tank_depth_mm = 7000;  // safety fallback
  ...
  prefs.putInt("tank_depth_mm", cfg_tank_depth_mm);
...
// -------------------- MODEM SESSION (robust) --------------------
bool modemOpenSession() {
  // flush
  while (SerialAT.available()) SerialAT.read();

  // basic AT
  String r = sendATGet("AT", 1000);
  if (r.indexOf("OK") == -1) {
    Serial.println("No AT OK");
    // try one more
    delay(200);
    r = sendATGet("AT", 1000);
  ...
  // SIM readiness (best effort)
  r = sendATGet("AT+CPIN?", 1500);
  Serial.println("CPIN: " + r);

  // registration check
  r = sendATGet("AT+CREG?", 1500);
  Serial.println("CREG: " + r);

  // signal quality
  r = sendATGet("AT+CSQ", 1000);
  Serial.println("CSQ: " + r);

  // set APN
  sendAT("AT+CGDCONT=1,\"IP\",\"" + apn + "\"", 1500);

  // attach
  r = sendATGet("AT+CGATT=1", 5000);
  if (r.indexOf("OK") == -1 && r.indexOf(": 1") == -1) {
    Serial.println("CGATT attach failed: " + r);
    // retry once
    delay(1000);
    r = sendATGet("AT+CGATT=1", 5000);
    if (r.indexOf("OK") == -1 && r.indexOf(": 1") == -1) {
      Serial.println("CGATT failed twice");
      return false;
    }
  }

  // open network
  r = sendATGet("AT+NETOPEN", 8000);
  if (r.indexOf("OK") == -1 && r.indexOf("ALREADY") == -1) {
    Serial.println("NETOPEN returned: " + r);
    // second attempt
    delay(500);
    r = sendATGet("AT+NETOPEN", 8000);
    if (r.indexOf("OK") == -1 && r.indexOf("ALREADY") == -1) {
      Serial.println("NETOPEN failed twice");
      return false;
    }
  }

  // initialize HTTP
  r = sendATGet("AT+HTTPINIT", 3000);
  if (r.indexOf("OK") == -1 && r.indexOf("ALREADY") == -1) {
    Serial.println("HTTPINIT failed: " + r);
    sendAT("AT+HTTPTERM", 1000);
    delay(200);
    r = sendATGet("AT+HTTPINIT", 3000);
    if (r.indexOf("OK") == -1) {
      Serial.println("HTTPINIT retry failed");
      return false;
    }
  }
  ...
  flushSerialATShort();
  Serial.println("Modem session opened.");
  return true;
}
...
// ensure session, attempt reopen if not attached
bool ensureModemSession(unsigned long timeoutMs = 10000) {
  String r = sendATGet("AT+CGATT?", 1500);
  if (r.indexOf(": 1") != -1) {
    return true;
  }
  Serial.println("ensureModemSession: not attached, reopening...");
  modemCloseSession();
  delay(500);
  return modemOpenSession();
}
...
// Assume sensor returns distance in mm
int readSensor() {
...
// distance_mm is distance from sensor to water surface in mm
String buildFullPayload(int distance_mm, bool pump_on, bool pump2_on) {
  // use configured tank depth
  int tankDepthMm = cfg_tank_depth_mm;
  if (tankDepthMm <= 0) tankDepthMm = 7000;  // safety fallback

  // tank level in mm (how much water in tank)
  int water_level_mm = tankDepthMm - distance_mm;

  // clamp to [0, tankDepthMm]
  if (water_level_mm < 0) water_level_mm = 0;
  if (water_level_mm > tankDepthMm) water_level_mm = tankDepthMm;

  // percentage (0–100%)
  float perc = 0.0f;
  if (tankDepthMm > 0) {
    perc = (float)water_level_mm * 100.0f / (float)tankDepthMm;
  }
  char percStr[16];
  ...
  if (gmtime_r(&now, &timeinfo) == nullptr) {
    ...
    snprintf(percStr, sizeof(percStr), "1970-01-01T00:00:00Z");
  }
  char ts[40];
  ...
  // Build JSON using configured strings
  String json = "{";
  ...
  json += "\"pumps\":[";
    ...
    json += "{";
      ...
      // Note: this reports the actual boolean value (true when pump_on==true)
  ...
  // level fields (mm only)
  json += "\"tank_depth_mm\":"   + String(tankDepthMm) + ",";
...
void sendJsonPost(const String &url, const String &payload) {
  if (url.length() == 0) {
    lastHttpStatus = -1000;
    lastHttpResp = "No URL configured";
    Serial.println(lastHttpResp);
    return;
  }

  // Ensure modem is attached; if not, try reopen
  if (!ensureModemSession()) {
    lastHttpStatus = -1001;
    lastHttpResp = "ensureModemSession failed";
    Serial.println(lastHttpResp);
    modemFailureCount++;
    return;
  }
  ...
  Serial.println("Payload len: " + String(len));
  ...
  if (resp.indexOf("DOWNLOAD") == -1 && resp.indexOf("OK") == -1) {
    ...
    Serial.println(lastHttpResp);
    // Attempt to recover HTTP stack and return; higher-level loop will try later
  ...
  while (millis() - start < 20000) { // wait up to 20s
    ...
    if (actionResp.indexOf("+HTTPACTION:") != -1) { gotAction = true; break; }
    server.handleClient();
    delay(5);
  }
  ...
  if (!gotAction) {
    ...
    Serial.println(lastHttpResp);
    // try to reinit network once
    modemCloseSession();
    delay(1000);
    if (!modemOpenSession()) {
      Serial.println("Reopen session failed after HTTP timeout.");
      modemFailureCount++;
    }
    return;
  } else {
  ...
  // success heuristics: if 200..299, clear failure counter
...
void sendSensorData() {
  int sensorVal = readSensor();   // distance in mm
  if (sensorVal < 0) {
...
void setup() {
  Serial.begin(115200);
  delay(100);
  Serial.println("ESP32 + A7670C: Modbus -> GSM POST (robust) with 2 pumps (mm only)");
  ...
  cfg_tank_depth_mm = prefs.getInt("tank_depth_mm", cfg_tank_depth_mm);
  ...
  if (!modemOpenSession()) {
    Serial.println("Initial modem session failed. Will attempt again in loop.");
  }
...
void loop() {
  ...
  if (millis() - lastHealthCheck >= HEALTH_CHECK_INTERVAL) {
    ...
    String csq = sendATGet("AT+CSQ", 1500);
    String cgatt = sendATGet("AT+CGATT?", 1500);
    Serial.println("Health check -> CSQ: " + csq + " | CGATT: " + cgatt);

    if (cgatt.indexOf(": 1") == -1) {
      Serial.println("CGATT lost -> try reopen session");
      if (!modemOpenSession()) {
        modemFailureCount++;
        Serial.println("modemOpenSession failed count=" + String(modemFailureCount));
      } else {
        modemFailureCount = 0;
      }
    } else {
      // reset failure count on healthy attach
      modemFailureCount = 0;
    }

    // if failures accumulate, reboot ESP (safe recovery)
    if (modemFailureCount >= MODEM_FAIL_MAX) {
      Serial.println("Too many modem failures, restarting ESP.");
      delay(200);
      ESP.restart();
    }
  }
  ...
  // forward modem output to USB serial for debug
  while (SerialAT.available()) Serial.write(SerialAT.read());
}
